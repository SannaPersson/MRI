...
def main(plot: bool, write_seq: bool, seq_filename: str = "gre_radial_half_four.seq"):
    # ======
    # SETUP
    # ======
    ...
    Nr_full = 256  # Number of radial spokes
    N_dummy = 20  # Number of dummy scans
    delta = np.pi / Nr_full  # Angular increment
    ...
    # CONSTRUCT SEQUENCE
    # ======
    alpha_fraction = 0.1 # 10-20% more than half
    Nr_half = int(np.ceil(Nr_full / 2 * (1 + alpha_fraction)))  # Adjusted number of spokes
    delta_half = 2 * np.pi / Nr_half  # New angular increment for half-Fourier acquisition

    for i in range(-N_dummy, Nr_half + 1):
    ...
    seq.set_definition(key="Ny", value=Nr)  # Number of radial spokes is equivalent to Ny
    seq.set_definition(key="Nz", value=1)   # Assuming a single slice acquisition
    # =========
    # WRITE .SEQ
    # =========
    if write_seq:
